...
module.exports = (env, callback) ->
  ...
  getArticles = (contents) ->
    ...
    articles = contents[options.articles]._.pages
    articles.sort (a, b) -> b.date - a.date
    return articles

  getUpcomming = (articles) ->
    # Helper that returns events comming in the future.
    now = (new Date()).getTime();
    return articles.filter (article) ->
        (new Date(article.date)).getTime() > now

  getPastEvents = (articles) ->
    # Helper that returns past events
    now = (new Date()).getTime();
    return articles.filter (article) ->
        (new Date(article.date)).getTime() <= now
  ...
  class PaginatorPage extends env.plugins.Page
    ...
    constructor: (@pageNum, @articles, @upcomming, @pastEvents) ->
    ...
    getView: -> (env, locals, contents, templates, callback) ->
      ...
      ctx = {events: @articles, @upcomming, @pastEvents, @prevPage, @nextPage}
  ...
  env.registerGenerator 'paginator', (contents, callback) ->
    ...
    upcomming = getUpcomming articles
    pastEvents = getPastEvents articles
    ...
    for i in [0...numPages]
      ...
      pages.push new PaginatorPage i + 1, pageArticles, upcomming, pastEvents
